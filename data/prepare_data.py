...
def make_windows(data, anno_df, anno_cols, winsec=30, sample_rate=SAMPLE_RATE, dropna=True, verbose=False, uniform_only=False):
    """ Segment accelerometer data into winsec*sample_rate length windows 
    
    Args:
        data: DataFrame with accelerometer data and annotations
        anno_df: DataFrame with annotation mappings
        anno_cols: List of annotation columns to extract
        winsec: Window size in seconds
        sample_rate: Sample rate in Hz
        dropna: If True, skip windows with all NA annotations
        verbose: If True, show progress bar
        uniform_only: If True, only include windows where all annotations are the same class
                     (no mixed-class windows). If False, use mode (majority class).
    """
    ...
    for t, w in tqdm(data.resample(f"{winsec}s", origin='start'), disable=not verbose):
        ...
        # Get non-NA annotations
        annot_valid = annot.dropna()
        
        if len(annot_valid) == 0:
            continue
        
        # Check for uniform class if required
        if uniform_only:
            unique_annotations = annot_valid.unique()
            if len(unique_annotations) != 1:
                # Mixed classes in this window, skip it
                continue
            y_anno_value = unique_annotations[0]
        else:
            # Use mode (original behavior)
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", message="Unable to sort modes")
                y_anno_value = annot.mode(dropna=False).iloc[0]
                
                if pd.isna(y_anno_value):  # skip if NA is the mode (majority unlabeled)
                    continue
        ...
        for col in anno_cols:
            if uniform_only:
                # All annotations are the same, so just take the first one
                y_col_value = anno_df.loc[annot_valid.iloc[0], f'label:{col}']
            else:
                y_col_value = anno_df.loc[annot_valid, f'label:{col}'].mode(dropna=False).iloc[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--outdir', '-o', default='prepared_data_1024_uniform')
    parser.add_argument('--annots', '-a', default='Walmsley2020,WillettsSpecific2018')
    parser.add_argument('--winsec', type=float, default=10.24)
    parser.add_argument('--n_jobs', type=int, default=8)
    parser.add_argument('--overwrite', action='store_true')
    parser.add_argument('--uniform_only', action='store_true',
                        help='Only include windows where all annotations are the same class (no mixed/transition windows)')
    ...
    load_all_and_make_windows(args.datadir, args.annots.split(","), args.outdir, args.n_jobs, 
                              args.overwrite, winsec=args.winsec, uniform_only=args.uniform_only)
