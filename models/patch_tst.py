...
from models.custom_encoder import RoPE_TransformerEncoderLayer
import math
...
class PatchTST(nn.Module):
    def __init__(self, config: dict):
        super().__init__()
        _pe = config.get("positional_encoding", "fixed")
        if not isinstance(_pe, str):
            raise TypeError(f"positional_encoding must be str, got {type(_pe)}")
        pe = _pe.lower().strip()
        if pe not in ("fixed", "learned", "rope"):
            raise ValueError(
                "positional_encoding must be 'fixed', 'learned', or 'rope', "
                f"got {config.get('positional_encoding')!r}"
            )
        config["positional_encoding"] = pe
        ...
        head_dim = config['d_model'] // config['t_heads']
        if pe == 'rope' and head_dim % 2 != 0:
            raise ValueError(
                f"RoPE requires even head_dim (d_model // t_heads); got head_dim={head_dim}"
            )
        ...
        if pe == 'rope':
            encoder_layer = RoPE_TransformerEncoderLayer(**enc_kw, max_seq_len=self.num_patches)
        else:
            encoder_layer = nn.TransformerEncoderLayer(**enc_kw)
        ...
        self.run = wandb.init(
            ...
            project="PatchTST_MLP_Classifier",
            config=config,
    ...
    def forward(self, x):
        ...
        if self.positional_encoding == 'fixed':
